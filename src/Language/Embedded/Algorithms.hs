...
-- whnf and normalForm based on Lambda Calculus Cooked Four Ways:
--
--   http://www.cs.uu.nl/wiki/pub/USCS/InterestingPapers/AugustsonLambdaCalculus.pdf

-- TODO whnf and normalForm should not hard code the application operator.

-- | Weak head normal form
--
-- TODO This function is untested and probably wrong.
whnf :: (Binding :<: f, App :<: f, Traversable f) => Term f -> Term f
whnf = go []
  where
    go env t
      | Just (Var v) <- project t
    ...
    go env t
      ...
      = case go env f of
          ...
          f' -> inject $ App f' a
    go env t = t  -- t is a constructor

-- | Normal form
--
-- TODO This function is untested and probably wrong.
normalForm :: (Binding :<: f, App :<: f, Traversable f) => Term f -> Term f
normalForm = go []
  where
    go env t
      | Just (Var v) <- project t
